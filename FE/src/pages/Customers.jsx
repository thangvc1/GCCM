...
export default function Customers() {
  const { customers, upsertCustomer, deleteCustomer, orders } = useStore();
  ...
  const spent = (id) =>
    orders.filter((o) => o.customerId === id && o.status !== "đã hủy").reduce((s, o) => s + o.total, 0);

  const save = (values) => {
    upsertCustomer({ ...form, ...values, id: form.id || uid("c"), points: +form.points || 0 });
    setForm(null);
  };
  ...
  return (
    <div>
      ...
      <div className="filters">
        <input className="search" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tên, SĐT" />
      </div>
              ...
              <tr key={c.id}>
                ...
                <td>
                  <div className="actions">
                    ...
                    {c.id !== "c0" && (
                      <button className="btn btn-sm btn-danger" onClick={() => confirm("Xóa khách?") && deleteCustomer(c.id)}>
                        Xóa
                      </button>
                    )}
                  </div>
      ...
      {form && (
        <Modal title={form.id ? "Sửa khách" : "Thêm khách"} onClose={() => setForm(null)}>
          <Form layout="vertical" initialValues={form} onFinish={save}>
            ...
            {form.id && <p className="sub">Tổng đã mua: {spent(form.id).toLocaleString("vi-VN")} đ</p>}
